...
def track():
    ...
    while True:
        ...
        if len(detections) > 0:
            aruco_to_track = detections[0] # only track 1 aruco
            
            print(aruco_to_track)

            aruco_center = aruco_to_track.Center # get center of aruco to track

            x_delta = vision.get_single_axis_delta(image_center[0],aruco_center[0]) # get x delta 
            y_delta = vision.get_single_axis_delta(image_center[1],aruco_center[1]) # get y delta

            lidar_on_target = vision.point_in_rectangle(image_center,aruco_to_track.Left, aruco_to_track.Right, aruco_to_track.Top, aruco_to_track.Bottom) #check if lidar is pointed on target
            ...
            #draw lidar distance
            prepare_visualisation(lidar_dist, aruco_center, aruco_to_track, image, yaw_command, x_delta, y_delta, fps,velocity_z_command, lidar_on_target)
        else:
            return "search"
...
def prepare_visualisation(lidar_distance, aruco_center, aruco_to_track, image, yaw_command, x_delta, y_delta, fps,velocity_x_command, lidar_on_target):
    ...
    #draw path
    cv2.line(image, (int(image_center[0]), int(image_center[1])), (int(aruco_center[0]), int(aruco_center[1])), (255, 0, 0), thickness=10, lineType=8, shift=0)

    #draw bbox around target
    cv2.rectangle(image,(int(aruco_to_track.Left),int(aruco_to_track.Bottom)), (int(aruco_to_track.Right),int(aruco_to_track.Top)), (0,0,255), thickness=10)

    #show drone center
    cv2.circle(image, (int(image_center[0]), int(image_center[1])), 20, (0, 255, 0), thickness=-1, lineType=8, shift=0)

    #show trackable center
    cv2.circle(image, (int(aruco_center[0]), int(aruco_center[1])), 20, (0, 0, 255), thickness=-1, lineType=8, shift=0)
